...
using kernel_params_32 = Instance_norm_kernel_params<uint16_t, uint16_t, 512, 8, 32>;
using kernel_params_32 = Instance_norm_kernel_params<uint16_t, uint16_t, 512, 8, 32>;
using kernel_params_64 = Instance_norm_kernel_params<uint16_t, uint16_t, 512, 16, 64>;

using kernel_params_32_int8 = Instance_norm_kernel_params<int8_t, int8_t, 512, 8, 32>;
using kernel_params_32_fp16_int8 = Instance_norm_kernel_params<uint16_t, int8_t, 512, 8, 32>;
...
__global__ __launch_bounds__(THREADS_PER_CTA)
    ...
    const int cta_nhw_regs = blockIdx.x * PIXELS_PER_CTA_IN_REGISTERS;
    // The position in the NHW dimension where the CTA starts for the portion in SMEM.
    const int cta_nhw_smem = blockIdx.x * PIXELS_PER_CTA_IN_SMEM;
        ...
        if( USE_ONLINE_APPROACH ) {
            ...
            for( int i = 0; i < PIXELS_PER_THREAD_IN_REGISTERS; ++i ) {
                ...
                if( idx < params.nhw && is_valid_c ) {
                    ldg(x_storage[i], &gmem_src[idx*stride_c_input], int8_in_scale);
                    is_valid[i] = 1.f;
                }
            }
    ...
    if( pixels_in_smem > 0 ) {
        ...
        for( int i = 0; i < PIXELS_PER_THREAD_IN_SMEM; ++i ) {
            ...
            zero(x_math);
            if( is_pixel_valid ) {
                ldg(x_math, &gmem_src[idx*stride_c_input], int8_in_scale);
            }

            // The offset to store in SMEM.
            const int offset = i*THREADS_PER_CTA*ELEMENTS_PER_LDG;
            // Store in SMEM.
            write_to_smem(&smem_storage[offset], threadIdx.x, x_math);
            // Update the count.
    ...
    int gmem_sums_offset = blockIdx.z*gridDim.y*gridDim.x*C_ELEMENTS_PER_CTA*2 
                         + blockIdx.y*gridDim.x*C_ELEMENTS_PER_CTA*2;
    ...
    int *gmem_counts = &params.gmem_counts[blockIdx.z*gridDim.y*gridDim.x + blockIdx.y*gridDim.x + blockIdx.x];
    if( threadIdx.x == 0 ) {
    ...
    int *gmem_retired_ctas = &params.gmem_retired_ctas[blockIdx.z * gridDim.y + blockIdx.y];
    // Register the CTA.
    int retired_ctas = -1;
    if( threadIdx.x == 0 ) {
        // Issue the membar.
        ...
        retired_ctas = atomicAdd(gmem_retired_ctas, 1);
    }
    ...
    // // DEBUG.
    // if( threadIdx.x < C_ELEMENTS_PER_CTA ) {
    //     printf("bidx=%d tidx=%d bias=%8.3f scale=%8.3f\n", 
    //        blockIdx.y, threadIdx.x, bias[0], scale[0]);
    // }
    // // END OF DEBUG.

    // Are all CTAs done?
    while( __syncthreads_and(retired_ctas != gridDim.x) ) {
        if( threadIdx.x == 0 ) {
    ...
    //     #pragma unroll
    //     for( int i = 0; i < ELEMENTS_PER_LDG; ++i ) {
    //         printf("bidx=%d tidx=%3d [%d] bias=%8.3f scale=%8.3f\n", 
    //             blockIdx.x, threadIdx.x, i, bias[i], scale[i]);
    //     }
    // }
    // // END OF DEBUG.
    ...
    for( int idx = threadIdx.x; idx < THREADS_PER_PIXEL*gridDim.x; idx += THREADS_PER_CTA ) {
        ...
        cta_count = __ldg(&params.gmem_counts[idx / THREADS_PER_PIXEL + blockIdx.z*gridDim.y*gridDim.x]); 
    ...
    int global_stats_offset = blockIdx.z * params.c;
    ...
    for( int loop_i = OUTER_LOOPS-1; loop_i >= 0; --loop_i ) {
        ...
        for( int i = 0; i < PIXELS_PER_THREAD_IN_REGISTERS; ++i ) {
            ...
            if( (unsigned) idx < params.nhw && is_valid_c ) {
                stg(&gmem_dst[idx*stride_c_output], x_math, int8_out_scale);
            }
        }
        ...
        for( int i = 0; i < PIXELS_PER_THREAD_IN_REGISTERS; ++i ) {
            const int idx = out_nhw + thread_in_cta_nhw + i*PIXELS_PER_LDG;
            if( (unsigned) idx < params.nhw && is_valid_c ) {
                ldg(x_storage[i], &gmem_src[idx*stride_c_output], int8_in_scale);
            }
        }
    }
    ...
    if( pixels_in_smem > 0 ) {
        for( int i = 0; i < PIXELS_PER_THREAD_IN_SMEM; ++i ) {
            ...
            const int offset = i*THREADS_PER_CTA*ELEMENTS_PER_LDG;
            float x_math[ELEMENTS_PER_LDG];
            read_from_smem(x_math, &smem_storage[offset], threadIdx.x);
...
dim3 instance_norm_grid_dim(InstanceNormFwdParams params)
{
    dim3 grid_dim;
    grid_dim.x = div_up(params.nhw, Kernel_params::PIXELS_PER_CTA);
    grid_dim.y = div_up(params.c, Kernel_params::C_ELEMENTS_PER_CTA);
    grid_dim.z = params.n;
...
template <typename Kernel_params>
void instance_norm_buffer_sizes(InstanceNormFwdParams params, 
                                size_t &size_sums, size_t &size_counts, size_t &size_retired_ctas)
{
    dim3 grid_dim = instance_norm_grid_dim<Kernel_params>(params);

    size_sums = grid_dim.z*grid_dim.y*grid_dim.x*Kernel_params::THREADS_PER_PIXEL*Kernel_params::ELEMENTS_PER_LDG*2*sizeof(float);
...
template <typename Kernel_params>
int instance_norm_fwd_launch(InstanceNormFwdParams params, cudaStream_t stream)
{
    dim3 grid_dim = instance_norm_grid_dim<Kernel_params>(params);

    size_t size_retired_ctas = grid_dim.z*grid_dim.y*sizeof(int);

    #define KERNEL_RUN(OUTER_LOOPS)  \
        {                            \
            ...
            instance_norm_fwd< \
                ...
                Kernel_params::ELEMENTS_PER_LDG, \
                Kernel_params::USE_ONLINE_APPROACH, \
                OUTER_LOOPS><<<grid_dim,Kernel_params::THREADS_PER_CTA, smem_size, stream>>>(params); }

    size_t smem_size = Kernel_params::PIXELS_PER_THREAD_IN_SMEM * 
                        Kernel_params::THREADS_PER_CTA * 
                        Kernel_params::ELEMENTS_PER_LDG * sizeof(float);
    
    int loop = 1;
    if( grid_dim.x <= params.sm_count ) {
        KERNEL_RUN(1);
    } else {
        grid_dim.x = params.sm_count;
        int PIXELS_PER_LDG = Kernel_params::THREADS_PER_CTA/Kernel_params::THREADS_PER_PIXEL;
        int nhw_in_regs = params.nhw - Kernel_params::PIXELS_PER_THREAD_IN_SMEM*PIXELS_PER_LDG*grid_dim.x;
        int pixels_per_iteration = Kernel_params::PIXELS_PER_THREAD_IN_REGISTERS*PIXELS_PER_LDG*grid_dim.x;
...
{
    //printf("ELEMENTS_PER_LDG = %d\n", kernel_params_32_fp16_int8::ELEMENTS_PER_LDG);
...
{
    if (input_data_type == 2 && output_data_type == 2) {
        return instance_norm_fwd_launch<kernel_params_32_int8>(params, stream);
    } else if (input_data_type == 1 && output_data_type == 2) {
        return instance_norm_fwd_launch<kernel_params_32_fp16_int8>(params, stream);
    } else if (input_data_type == 1 && output_data_type == 1) {
        if (params.c <= c_cond_g) {
            return instance_norm_fwd_launch<kernel_params_32>(params, stream);
        }
        else {
            return instance_norm_fwd_launch<kernel_params_64>(params, stream);
        }
    } else {
        fprintf(stderr, "Unsupported format combination by the instance norm kernel\n");
        assert(0);
