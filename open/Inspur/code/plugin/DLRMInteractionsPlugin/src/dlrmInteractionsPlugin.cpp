...
int DLRMInteractionsPlugin::initialize()
{
    if (!mInitialized)
    {
        ...
        {
            // This GPU doesn't have device data initialized, but some other has. Host data should be initialized by this point.
        ...
        else
        {
            ...
            // No GPUs have device data initialized, no host data should be initialized then at this point
            ASSERT(mHostData.mCounter == 0);
            ...
            if (useRowFrequencies)
            {
                ...
                int embeddingTables;
                input.read((char *)&embeddingTables, sizeof(int));

                std::vector<std::pair<int, float>> rowIdFreqList(mEmbeddingRows);
                int baseRowId = 0;
                for(int feature_id = 0; feature_id < embeddingTables; ++feature_id)
                {
                    int tableRows;
                    input.read((char *)&tableRows, sizeof(int));
                    double freqSum = 0.0;
                    for(int tableRowId = 0; tableRowId < tableRows; ++tableRowId)
                    {
            ...
            {
                int embeddingRowsOnHost = static_cast<int>(mEmbeddingRows * (1.0F - mEmbeddingWeightsOnGpuPart));
                ...
                mHostData.mEmbeddingsScales.resize(embeddingTables);
                input.read((char *)&mHostData.mEmbeddingsScales.front(), sizeof(float) * embeddingTables);

                // Load embeddings from file to GPU memory in bacthes to reduce peak usage of host memory
...
DimsExprs DLRMInteractionsPlugin::getOutputDimensions(int outputIndex, const DimsExprs* inputs, int nbInputs, IExprBuilder& exprBuilder)
{
    ...
    if (mInteractionsOutputInterleaved)
        return DimsExprs{4, {exprBuilder.operation(DimensionOperation::kCEIL_DIV, *inputs[0].d[0], *exprBuilder.constant(2)),
            exprBuilder.constant((embeddingSize + (interactionFeatures * (interactionFeatures - 1) / 2) + 31) / 32 * 32),
...
{
    return sizeof(int) * 8 + sizeof(float) * 3 + MAX_TABLE_OFFSETS * sizeof(int) + MAX_FILEPATH_LENGTH * 2;
}
