...
namespace lwis {
  ...
  void Device::Completion() {
    ...
    while (true) {
      ...
      if (batch.SampleIds.empty()) break;

      // wait on event completion
      CHECK_EQ(cudaEventSynchronize(batch.Event), cudaSuccess);

      // copy result to staging buffer, assuming contiguous and same sizes across samples
      size_t numResponses{batch.SampleIds.size()};
      size_t responseSize{batch.ResponseSize};
      size_t stagingBufSize{numResponses * responseSize};
      stagingBuf.resize(stagingBufSize);
      std::memcpy(stagingBuf.data(), batch.ResponseBuf, stagingBufSize);

      // release this stream early
      m_StreamQueue.emplace_back(batch.Stream);

      // construct QuerySampleResponces
      std::vector<mlperf::QuerySampleResponse> responses(numResponses);
      for (size_t i = 0; i < numResponses; i++)
      {
        responses[i].id = batch.SampleIds[i];
        responses[i].data = reinterpret_cast<uintptr_t>(stagingBuf.data() + responseSize * i);
        responses[i].size = responseSize;
      }

      // callback if it exists
      if (m_ResponseCallback)
      {
        CHECK(batch.SampleIndices.size() == batch.SampleIds.size()) << "missing sample IDs";
        m_ResponseCallback(responses.data(), batch.SampleIndices, numResponses);
      }

      // assume this function is reentrant for multiple devices
      TIMER_START(QuerySamplesComplete);
      mlperf::QuerySamplesComplete(responses.data(), numResponses);
      ...
      for (auto &deviceTypes : params.EngineNames) {
        ...
        for (auto &batches : deviceTypes) {
          ...
          for (int32_t deviceInstance = 0; deviceInstance < (isDlaDevice ? runtime->getNbDLACores() : 1); deviceInstance++) {
            ...
            auto device = std::make_shared<lwis::Device>(deviceNum, numCopyStreams, numInferStreams, m_ServerSettings.CompleteThreads, m_ServerSettings.EnableSpinWait, m_ServerSettings.EnableDeviceScheduleSpin, batchSize, isDlaDevice);
  ...
  void Server::IssueBatch(DevicePtr_t device, size_t batchSize, std::deque<mlperf::QuerySample>::iterator begin, std::deque<mlperf::QuerySample>::iterator end, cudaStream_t copyStream) {
    ...
    if (m_ServerSettings.EnableResponse) {
      ...
      Batch batch;
      batch.ResponseBuf = static_cast<int8_t *>(bufferManager->getHostBuffer(bindingIdx));
      batch.ResponseSize = volume(engine->getBindingDimensions(bindingIdx), engine->hasImplicitBatchDimension())
        * getElementSize(engine->getBindingDataType(bindingIdx));
      batch.SampleIds.reserve(std::distance(begin, end));
      batch.SampleIndices.reserve(std::distance(begin, end));
      for (auto it = begin; it != end; ++it) {
        batch.SampleIds.emplace_back(it->id);
        if (device->m_ResponseCallback) batch.SampleIndices.emplace_back(it->index);
      }
      batch.Event = dtoh;
  ...
  std::vector<void *> Server::CopySamples(DevicePtr_t device, size_t batchSize, std::deque<mlperf::QuerySample>::iterator begin, std::deque<mlperf::QuerySample>::iterator end, cudaStream_t stream, bool directHostAccess, bool staging) {
    ...
    if (m_SampleLibrary) {
      ...
      for (size_t i = 0; i < num_inputs; i++) {
        ...
        CHECK(m_SampleLibrary->GetSampleSize(i) == sampleSize) << "Sample size (" << m_SampleLibrary->GetSampleSize(i)
      ...
      if (!m_ServerSettings.ForceContiguous) {
        for (size_t i = 0; i < num_inputs && contiguous; i++) {
          auto prev = static_cast<int8_t *>(m_SampleLibrary->GetSampleAddress(begin->index, i, deviceId));
          for (auto it = begin + 1; it != end; ++it) {
            auto next = static_cast<int8_t *>(m_SampleLibrary->GetSampleAddress(it->index, i, deviceId));
            if (next != prev + m_SampleLibrary->GetSampleSize(i)) {
              contiguous = false;
              break;
            }
            prev = next;
          ...
          for (auto it = begin; it != end; ++it) {
            if (m_ServerSettings.EnableStartFromDeviceMem) {
              ...
              CHECK_EQ(cudaMemcpyAsync(
                static_cast<int8_t *>(bufferManager->getDeviceBuffer(i)) + offset++ * m_SampleLibrary->GetSampleSize(i),
                m_SampleLibrary->GetSampleAddress(it->index, i, deviceId),
                m_SampleLibrary->GetSampleSize(i), cudaMemcpyDeviceToDevice, stream), cudaSuccess);
              ...
              memcpy(static_cast<int8_t *>(bufferManager->getHostBuffer(i)) + offset++ * m_SampleLibrary->GetSampleSize(i), m_SampleLibrary->GetSampleAddress(it->index, i), m_SampleLibrary->GetSampleSize(i));
              ...
              bufferManager->copyInputToDeviceAsync(i, stream, m_SampleLibrary->GetSampleAddress(it->index, i), m_SampleLibrary->GetSampleSize(i), offset++);
              device->m_Stats.m_PerSampleCudaMemcpyCalls++;
            }
          }
        }
          ...
          if (m_ServerSettings.EnableStartFromDeviceMem) {
            if (!m_ServerSettings.EnableCudaGraphs) {
              ...
              for (int profile_idx = 0; profile_idx < engine->getNbOptimizationProfiles(); profile_idx++) {
                buffers[i + profile_idx * num_bindinges_per_profile] = m_SampleLibrary->GetSampleAddress(begin->index, i, deviceId);
              }
            }
            else {
              ...
              CHECK_EQ(cudaMemcpyAsync(
                ...
                m_SampleLibrary->GetSampleAddress(begin->index, i, deviceId),
                m_SampleLibrary->GetSampleSize(i) * batchSize, cudaMemcpyDeviceToDevice, stream), cudaSuccess);
            ...
            if (!m_ServerSettings.EnableCudaGraphs) {
              // access samples directly when they are contiguous
              for (int profile_idx = 0; profile_idx < engine->getNbOptimizationProfiles(); profile_idx++) {
                buffers[i + profile_idx * num_bindinges_per_profile] = m_SampleLibrary->GetSampleAddress(begin->index, i);
              }
            }
            else {
              // with cuda graphs, we have to do H2H copies because we cannot change bindings for graphs
              memcpy(bufferManager->getHostBuffer(i), m_SampleLibrary->GetSampleAddress(begin->index, i), m_SampleLibrary->GetSampleSize(i) * batchSize);
            }
          }
          else {
            // copy direct to device buffer with single DMA
            bufferManager->copyInputToDeviceAsync(i, stream, m_SampleLibrary->GetSampleAddress(begin->index, i), m_SampleLibrary->GetSampleSize(i) * batchSize);
            device->m_Stats.m_BatchedCudaMemcpyCalls++;
          }
        }
      }
