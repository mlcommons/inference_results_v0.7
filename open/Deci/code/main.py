...
SUPPORTED_DATASETS = {
    "imagenet_deci":
        (imagenet.Imagenet, dataset.pre_process_deci, dataset.PostProcessArgMax(offset=0),
...
SUPPORTED_PROFILES = {
    ...
    ######## DECI MODELS ########
    "deci_model1-openvino": {
        "dataset": "imagenet_deci",
        "backend": "openvino",
        "data-format": "NCHW",
        "max-batchsize": 1,
        "model-name": "deci-model",
        "threads": 1
    },
    "deci_model64-openvino": {
        "dataset": "imagenet_deci",
        "backend": "openvino",
        "data-format": "NCHW",
        "max-batchsize": 64,
...
def main():
    ...
    toc = time.time()
    print("Warmup time = {}".format(toc-tic))
    ...
    samples_to_cache= 1024
    if args.accuracy:
        settings.mode = lg.TestMode.AccuracyOnly
        # FIXME: in case there is not enough RAM
        # samples_to_cache = count
    ...
    qsl = lg.ConstructQSL(count, min(count, samples_to_cache), ds.load_query_samples, ds.unload_query_samples)
