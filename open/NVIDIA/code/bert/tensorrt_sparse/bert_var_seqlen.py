...
from code.bert.tensorrt_sparse.builder_utils import BertConfig, load_onnx_fake_quant, load_pickle_dictionary
from code.bert.tensorrt_sparse.int8_builder_var_seqlen import bert_squad_int8_var_seqlen
from code.bert.tensorrt_sparse.int8_builder_vs_il import bert_squad_int8_vs_il
...
class BERTBuilder(BenchmarkBuilder):
    ...
    def __init__(self, args):
        ...
        self.bert_config_path = "code/bert/tensorrt_sparse/bert_config.json"
        ...
        if self.is_int8:
            self.model_path = dict_get(args, "model_path", default='build/models/bert/megatron.pkl')

        self.bert_config = BertConfig(self.bert_config_path)

        self.use_sparsity = 'sparsity' in args and args['sparsity'] == True
    ...
    def build_engines(self):
        ...
        # Load weights
        weights_dict = load_pickle_dictionary(self.model_path)
        ...
        with self.builder.create_network(1 << int(trt.NetworkDefinitionCreationFlag.EXPLICIT_BATCH)) as network:
            ...
            if self.is_int8:
                ...
                raise RuntimeError("FP16 sparse BERT not supported!")

            if self.use_sparsity:
                logging.info("Set Sparsity Flag")
                self.builder_config.set_flag(trt.BuilderFlag.SPARSE_WEIGHTS)
