...
class ResNet50(BenchmarkBuilder):
    ...
    def initialize(self):
        ...
        self.gpu_arch = None
        if "T4" in self.system_id or "TitanRTX" in self.system_id:
            self.gpu_arch = "Turing"
        elif "V100" in self.system_id or "DGX-2H" in self.system_id:
            self.gpu_arch = "Volta"
        elif "Xavier" in self.system_id:
            self.gpu_arch = "Xavier"
        elif "A100" in self.system_id:
            self.gpu_arch = "Ampere"

        # Turn on res2-full-fusion plugin only for Turing now.
        self.use_res2_mega_kernel = (self.gpu_arch == "Turing") or (self.gpu_arch == "Ampere")
        ...
        if self.device_type == "gpu" and self.precision == "int8" and not self.need_calibration:
            ...
            if not self.use_res2_mega_kernel:
