...
{
    return max(min(__float2int_rn(v), 127), -127);
}
...
#pragma unroll
    ...
    // Store dense features
    const int OUTPUT_SAMPLE_OFFSET_INT = OUTPUT_INTERLEAVED ? 8 : 32;
    if (laneId < K_BLOCKS * 4)
#pragma unroll
        for(int sampleId = 0; sampleId < SAMPLES_PER_WARP; ++sampleId)
...
#pragma unroll
    ...
    const int OUTPUT_SAMPLE_OFFSET_BYTE = OUTPUT_INTERLEAVED ? 32 : 128;
#pragma unroll
    for (int i = 0; i < M_BLOCKS; i++) 
#pragma unroll
        for (int j = 0; j < M_BLOCKS; j++)
