...
def bert_encoder_layer_int8_var_seqlen(cfg, max_seqlen, weights_dict, network, input_tensor, cu_seqlens, layer, mask):
    ...
    assert(weights_dict[prefix + 'attention_self_query_input_amax'] == weights_dict[prefix + 'attention_output_add_residual_input_quantizer_amax'])
    assert(weights_dict[prefix + 'intermediate_dense_act_input_quantizer_amax'] == weights_dict[prefix + 'output_add_residual_input_quantizer_amax'])
    ...
    ##### FC QKV
    dr_qkv = max(
        weights_dict[prefix + 'attention_self_matmul_q_input_quantizer_amax'],  # attention_self_qv_a_input_quantizer_amax
        weights_dict[prefix + 'attention_self_matmul_k_input_quantizer_amax'],  # attention_self_qv_b_input_quantizer_amax
        weights_dict[prefix + 'attention_self_matmul_v_input_quantizer_amax'],  # attention_self_av_b_input_quantizer_amax
    )
    ...
    ##### QKV2CTX
    dr_probs = weights_dict[prefix + 'attention_self_matmul_a_input_quantizer_amax']  # attention_self_av_a_input_quantizer_amax
    ...
    ##### Skip-Layernorm 1
    dr_skln1 = weights_dict[prefix + 'intermediate_dense_act_input_quantizer_amax'] # intermediate_dense_input_amax
...
def bert_squad_int8_var_seqlen(network, weights_dict, cfg, use_sparsity, input_shape, cu_seqlens_shape):
