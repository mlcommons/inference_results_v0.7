...
namespace lwis {
  ...
  struct Batch { 
    ...
    std::vector<mlperf::QuerySampleIndex> SampleIndices;
    cudaEvent_t Event;
    cudaStream_t Stream;
  };
  ...
  public:
    Device(size_t id, size_t numCopyStreams, size_t numInferStreams, size_t numCompleteThreads, bool enableSpinWait, bool enableDeviceScheduleSpin, size_t batchSize, bool dla) : m_Id(id), m_BatchSize(batchSize), m_EnableSpinWait(enableSpinWait), m_EnableDeviceScheduleSpin(enableDeviceScheduleSpin), m_DLA(dla), m_CopyStreams(numCopyStreams), m_InferStreams(numInferStreams) {
