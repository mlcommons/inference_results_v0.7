...
constexpr int BERT_CUDA_GRAPH_SIZE = 12; // in MB
...
size_t BERTCoreVS::GetTotalGPUMemory() {
...
static std::vector<BERTGraphSpec_t> ParseGraphSpecs(const std::string& graphSpecs) {
    // Parse a list of (uint, uint, uint), simply ignore parenthesis and spaces
    ...
    for(size_t pos = 0; pos < graphSpecs.size(); ++pos) {
        if (!std::isdigit(graphSpecs[pos])) {
            if (std::isdigit(graphSpecs[prev])) {
                switch (count) {
                    case 0:
                        std::get<0>(newGraphSpec) = std::stoi(graphSpecs.substr(prev, pos-prev));
                        break;
                    case 1:
                        std::get<1>(newGraphSpec) = std::stoi(graphSpecs.substr(prev, pos-prev));
                        break;
                    case 2:
                        std::get<2>(newGraphSpec) = std::stoi(graphSpecs.substr(prev, pos-prev));
                        break;
                }
                ++count;
            }
        ...
        if (count == 3) {
            retGraphSpecs.push_back(newGraphSpec);
            count = 0;
        }
    }
...
void BERTCoreVS::InitializeGraphSpecs(int graphMaxSeqLen, const std::string& graphSpecs, int numBERTCores) {
    ...
    if (!graphSpecs.empty()) {
        LOG(INFO) << "CUDA graph specs: " << graphSpecs;
        tmpGraphSpecs = ParseGraphSpecs(graphSpecs);
    } else if (mMaxBatchSize == 1) {
        int step = 4;
...
void BERTCoreVS::BuildGraphs(int graphMaxSeqLen, const std::string& graphSpecs, int numBERTCores)
{
    ...
    int numGraphs = 0;
    // build one graph for each copyStream, (batch size, total length)
    for (int sidx = 0; sidx < numCopyStreams; ++sidx) {
        for (const auto& spec : mGraphSpecs) {
            int maxSeqLen = std::get<0>(spec);
            int batchSize = std::get<1>(spec);
            int totalSeqLen = std::get<2>(spec);
            ...
            // need to issue enqueue to TRT to setup resources properly _before_ starting graph construction
...
BERTCoreVS::~BERTCoreVS()
{
    ...
    if (!mQuantizedPoints.empty()) {
        // the idea is that we can put this list as "graph_specs" in config.json
        LOG(INFO) << "Quantized points: " << mQuantizedPoints.size();
        char * pointsStr = new char[mQuantizedPoints.size() * 20];
        size_t pos = 0;
        for (auto & v : mQuantizedPoints) {
            auto& [maxSL, BS, totSL] = v;
            auto n = snprintf(pointsStr + pos, 20, "(%d, %d, %d), ", maxSL, BS, totSL);
            pos += n;
        }
        if (pos >= 2) {
          pointsStr[pos-2] = '\0';
