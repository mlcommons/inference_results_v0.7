...
from code.bert.tensorrt_sparse.helpers.data_processing import read_squad_json, convert_example_to_features
from code.bert.tensorrt_sparse.helpers.tokenization import BertTokenizer
...
def preprocess_bert(data_dir, model_dir, preprocessed_data_dir):
    ...
    input_ids = np.zeros((len(eval_features), max_seq_length), dtype=np.int32)
    input_mask = np.zeros((len(eval_features), max_seq_length), dtype=np.int32)
    segment_ids = np.zeros((len(eval_features), max_seq_length), dtype=np.int32)
    for idx, feature in enumerate(eval_features):
        print("Processing {:d}/{:d}...".format(idx, len(eval_features)))
...
def main():
    ...
    parser.add_argument(
        "--model_dir", "-m",
        help="Specifies the directory containing the models.",
