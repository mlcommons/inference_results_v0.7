...
template<int N>
DEVICE_FUNCTION void from_float(int (&dst)[N], const float (&src)[4*N], float scale) {
    union Pack_t {
        int8_t x[4];
        int val;
    };
...
template< int N >
DEVICE_FUNCTION void to_float(float (&dst)[4*N], int (&src)[N], float scale) {
    union Pack_t {
        int8_t x[4];
        int val;
    };
    ...
    for( int i = 0; i < N; ++i ) {
        ...
        #pragma unroll
        for (int ii = 0; ii < 4; ++ii) {
            dst[4*i+ii] = __int2float_rn((int)packed.x[ii]) * scale;
            //dst[4*i+ii] = (float)packed.x[ii];
        }
    }
}
...
template <typename T>
DEVICE_FUNCTION void ldg(int (&dst)[2], const T *gmem) {
...
template< int N >
DEVICE_FUNCTION void ldg(float (&dst)[N], const uint16_t *gmem, float scale) {
    int tmp[N/2];
    ldg(tmp, gmem);
    to_float(dst, tmp);
}
...
template< int N >
DEVICE_FUNCTION void ldg(float (&dst)[N], const int8_t *gmem, float scale) {
    int tmp[N/4];
    ldg(tmp, gmem);
    to_float(dst, tmp, scale);
}
...
template <typename T>
DEVICE_FUNCTION void stg(T *gmem, int (&src)[2]) {
...
/*DEVICE_FUNCTION void write_to_smem(float *smem, int idx, const float (&x)[8]) {
    ...
    reinterpret_cast<float4*>(&smem[12*idx])[0] = make_float4(x[4], x[5], x[6], x[7]);
}*/
