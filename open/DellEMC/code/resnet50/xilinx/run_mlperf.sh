...
do
  RESULT_DIR="results/resnet50/${SCENARIO}"
  AUDIT_DIR="audit/resnet50/${SCENARIO}"
  ...
  do
    ...
    ./run.sh -s ${SCENARIO} -m PerformanceOnly ${RUN_PARAMS} -r ${TARGET_QPS}
    cp mlperf_log*.{txt,json} ${RESULT_DIR}/performance/run_${ITER}
  done
  echo -e "\nRunning ${SCENARIO} accuracy"
  ./run.sh -s ${SCENARIO} -m AccuracyOnly ${RUN_PARAMS} -n 50000 -r ${TARGET_QPS}
