...
class RequestPool
{
  public:
    struct Block
    {
        Block() : m_NextBlock(nullptr), m_Data(nullptr) {}
        Block* m_NextBlock;
    ...
    static Block* Obtain()
    {
        Block* res;
        {
            std::lock_guard<std::mutex> lk(instance_->m_ListMtx);
            res = instance_->m_Head;
            if(res != nullptr)
            {
                instance_->m_Head = instance_->m_Head->m_NextBlock;
            }
            else
            {
                instance_->m_Blocks.emplace_back();
                res = &instance_->m_Blocks.back();
            }
        }
        return res;
    }
    ...
    static void Release(Block* block)
    {
        {
            std::lock_guard<std::mutex> lk(instance_->m_ListMtx);
            block->m_NextBlock = instance_->m_Head;
            instance_->m_Head = block;
        }
    }

    static void InitInferenceRequest(Block* block);
  ...
  private:
    ...
    static std::unique_ptr<RequestPool> instance_;
...
class Server_SUT : public mlperf::SystemUnderTest
{
  public:
    Server_SUT(std::string name, std::string model_repo_path, std::string model_name,
        ...
        : m_Name(name), m_ModelRepositoryPath(model_repo_path), m_ModelName(model_name),
          ...
          m_InputMemoryType(start_from_device ? TRITONSERVER_MEMORY_GPU : TRITONSERVER_MEMORY_CPU_PINNED)
    {
    }
    ~Server_SUT() {}
  ...
  private:
    ...
    std::vector<std::vector<char>> m_InputPaddings;
