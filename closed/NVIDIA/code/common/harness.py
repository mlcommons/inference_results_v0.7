...
plugin_map = {
    ...
    BENCHMARKS.DLRM: ["build/plugins/DLRMInteractionsPlugin/libdlrminteractionsplugin.so", "build/plugins/DLRMBottomMLPPlugin/libdlrmbottommlpplugin.so"],
    BENCHMARKS.SSDMobileNet: ["build/plugins/NMSOptPlugin/libnmsoptplugin.so"],
    BENCHMARKS.UNET: ["build/plugins/instanceNormalization3DPlugin/libinstancenorm3dplugin.so", "build/plugins/pixelShuffle3DPlugin/libpixelshuffle3dplugin.so"]

}

scenario_result_regex = {
    SCENARIOS.SingleStream: r"([0-9]+th percentile latency \(ns\) +: [0-9\.]+)",
    SCENARIOS.MultiStream: r"(Samples per query : [0-9\.]+)",
    SCENARIOS.Offline: r"(Samples per second: [0-9e\+\.]+)",
    SCENARIOS.Server: r"(Scheduled samples per second +: [0-9e\+\.]+)",
}
