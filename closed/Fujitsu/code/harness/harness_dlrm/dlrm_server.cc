    ...
    if (!startFromDevice)
    {
        ...
        for (int i = 0; i < numBatchMakers; ++i)
        {
            ...
            if (UseNuma())
            {
                ...
                long ret = set_mempolicy(MPOL_BIND, &nodeMask, mNumaConfig.size() + 1);
            ...
            mBatchMakers.emplace_back(std::make_shared<BatchMaker>(
                ...
                /* qsl = */ qsl,
                /* cpus = */ UseNuma() ? mNumaConfig[i] : std::vector<int>()));
    ...
    for(const auto& deviceId : gpus)
    {
        for (size_t profileIdx = 0; profileIdx < numDLRMCores; ++profileIdx)
        {
            ...
            if (UseNuma())
            {
                cpu_set_t cpuset;
                CPU_ZERO(&cpuset);
                for(int cpu : mNumaConfig[deviceId])
                {
                    CPU_SET(cpu, &cpuset);
                }
                CHECK_EQ(pthread_setaffinity_np(mWorkerThreads.back().native_handle(), sizeof(cpu_set_t), &cpuset), 0);
            }
        }
    }
}
...
void DLRMServer::ProcessTasks(std::shared_ptr<DLRMCore> dlrmCore, int deviceId, int profileIdx)
{
    ...
    while(true)
    {
        ...
        int batchMakerIdx = UseNuma() ? deviceId : 0;
