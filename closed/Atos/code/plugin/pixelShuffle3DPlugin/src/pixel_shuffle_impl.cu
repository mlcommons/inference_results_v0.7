...
pixel_shuffle_ncdhw32_to_ncdhw32(PixelShuffleParams params)
{
    ...
    int dhw = opq / RST;
    int hw = pq / (S * T);
    const int global_batch_offset = blockIdx.z *params.k * opq;
    ...
    const int8_t *gmem_src = &reinterpret_cast<const int8_t *>(params.gmem_src)[ global_batch_offset
                                                                                + d * hw * 32
                                                                                + h * params.q / T * 32];

    int8_t *gmem_dst = &reinterpret_cast<int8_t *>(params.gmem_dst)[ global_batch_offset 
                                                                                + cta_k * 32 * opq
...
pixel_shuffle_ncdhw_to_ncdhw(PixelShuffleParams params)
{
    ...
    // The base pointer to load from.
    const typename Kernel_params::Data_Type *gmem_src = &reinterpret_cast<const typename Kernel_params::Data_Type *>(params.gmem_src)[ global_batch_offset
                                                                                + d * hw 
                                                                                + h * params.q / T + w];

    typename Kernel_params::Data_Type *gmem_dst = &reinterpret_cast<typename Kernel_params::Data_Type *>(params.gmem_dst)[ global_batch_offset 
                                                                                + cta_k * opq
                                                                                + opq_idx];
    int c = k * RST + (o % R) * (S*T) + (p % S) * T + q % T;
    int is_valid_c = (c < params.k * RST);
