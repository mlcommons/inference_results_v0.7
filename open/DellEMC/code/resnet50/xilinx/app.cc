...
public:
  ...
  : image_(image) {
    if (posix_memalign(&data_, getpagesize(), size))
      throw std::bad_alloc();
  }
...
public:
  ...
  : runner_tbuf_idx_(0)
  {
    ...
    const unsigned tbufPoolSize  = 64;
    ...
    for (unsigned bi=0; bi < tbufPoolSize; bi++)
    {
      ...
      runner_tbufs_.push_back(std::make_pair(inputs, outputs));
  ...
  void execBatch(std::vector<QuerySample> batch) {
    ...
    auto &outTensors = runner_tbufs_[tbufIdx].second;
    ...
    std::vector<std::unique_ptr<xir::vart::CpuFlatTensorBuffer> > tbufs;
    std::vector<xir::vart::TensorBuffer*> inTensors;
    for (int bi=0; bi < batch.size(); bi++) 
    {
      auto &input = imageCache.get(batch[bi].index);
      std::unique_ptr<xir::vart::CpuFlatTensorBuffer> tbuf(
        new xir::vart::CpuFlatTensorBuffer(input.data(), 
        runner_tbufs_[tbufIdx].first[bi]->get_tensor()));
      inTensors.emplace_back(tbuf.get());
      tbufs.emplace_back(std::move(tbuf));
    }

    // execute on FPGA
    auto job_id = runner_->execute_async(inTensors, outTensors);
    ...
    for (int i=0; i < batch.size(); i++) 
    {
      void *outData;
      size_t sz;
      std::tie(outData, sz) = outTensors[i]->data();
      const int8_t *fc = (const int8_t*)outData;

      resData[i] = Top1(fc, outSize);
  ...
  int getWidth() {
    ...
    return inputTensors[0]->get_dim_size(2); // NHWC
  }
  int getHeight() {
    auto inputTensors = runner_->get_input_tensors();
    return inputTensors[0]->get_dim_size(1); // NHWC
  }
...
private:
  ...
  std::vector<const xir::vart::Tensor *> input_tensors_;
  std::vector<const xir::vart::Tensor *> output_tensors_;

  // tensorbuffer pool for each Runner -- alloc once and reuse across queries
  std::vector<
    std::pair<std::vector<xir::vart::TensorBuffer*>, 
              std::vector<xir::vart::TensorBuffer*> > > runner_tbufs_;
