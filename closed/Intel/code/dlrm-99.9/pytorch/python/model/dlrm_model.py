...
def fuse_model(layers):
    ...
    for i in range(0, list_length, 2):
        if isinstance(layers[i], nn.Linear):
            LL = ipex.LinearFuseRelu(int(layers[i].in_features), int(layers[i].out_features), bias=True, fuse_relu=isinstance(layers[i+1], nn.ReLU))
...
def model_util(model, server):
    ...
    if server:
        return prepack_weight(model).cpu().share_memory().to('dpcpp')
    else:
        return prepack_weight(model.to('dpcpp'))
