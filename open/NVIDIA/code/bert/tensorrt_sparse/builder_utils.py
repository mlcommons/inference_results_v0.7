...
def onnx_to_tf_name(onnx_name):
    ...
    if toks[0] == 'bert': #embeddings or encoder
        if toks[1] == 'encoder': #transformer
            if toks[-2] == 'dense_act': #replace dense_act with dense
                toks[-2] = 'dense'
...
def load_onnx_fake_quant(path):
    model = onnx.load(path)
    weights = model.graph.initializer
    # masks are stored in int8
    weights_dict = dict([(onnx_to_tf_name(w.name), np.frombuffer(w.raw_data, np.int8).reshape(w.dims)) if w.name.split('_')[-1] == 'mask'
                        else (onnx_to_tf_name(w.name), np.frombuffer(w.raw_data, np.float32).reshape(w.dims)) for w in weights])
    return weights_dict
