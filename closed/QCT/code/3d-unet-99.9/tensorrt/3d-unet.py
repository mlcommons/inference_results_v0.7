...
class UnetBuilder(BenchmarkBuilder):
    ...
    # touchup ONNX network with graph surgeon
    def preprocess_onnx(self, model):
        graph = gs.import_onnx(model)
        if self.use_instnorm3d_plugin:
            for node in graph.nodes:
                # Replace InstancrNormalization with INSTNORM3D_TRT
            ...
            # Add fused leaky relu, remove leaky-relu nodes
        ...
        # Remove the four unnessary outputs.
        graph.outputs = [output for output in graph.outputs if output.name == "output"]

        # Remove dead nodes.
        graph.cleanup().toposort()

        # this is toposorted; 3D UNet is pretty linear in order so I can add name here
        uniq_num = 0
