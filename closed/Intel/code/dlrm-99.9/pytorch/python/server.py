...
class Item:
    ...
    def __init__(self, query_id, content_id, issues):
...
class Consumer(multiprocessing.Process):
    ...
    def __init__(self, task_queue, result_queue, ds_queue, lock, init_counter, total_samples, finished_samples, barrier, socket_id, args, min_query_count):
        ...
        self.workers = []
        if self.socket_id == 0:
            self.threads = cpus_per_socket - cpus_per_instance
            self.affinity = range(2, round(cpus_per_socket))
        else:
            self.threads = cpus_per_socket
            self.affinity = range(round(socket_id*cpus_per_socket), round((socket_id+1)*cpus_per_socket))
    ...
    def warmup(self, ds, model):
        global cpus_per_socket
        global cpus_per_instance
        instance_per_socket = cpus_per_socket // cpus_per_instance
        import torch
        for _ in range(3):
            for s  in range(instance_per_socket * 100, instance_per_socket * 700 + 100, 100):
    ...
    def handle_tasks(self, model, task_queue, result_queue, i, socket_id, args):
        import torch
        from intel_pytorch_extension import core
        global cpus_per_socket
        global cpus_per_instance
        global total_instances
        if self.socket_id == 0 and cpus_per_instance >= (cpus_per_socket // 2):
            if i == 0:
                core.thread_bind(socket_id, cpus_per_socket, 2, cpus_per_instance - 2)
            else:
                core.thread_bind(socket_id, cpus_per_socket, (i - 1) * cpus_per_instance + cpus_per_instance, cpus_per_instance)
        elif self.socket_id == 0:
            core.thread_bind(socket_id, cpus_per_socket, i * cpus_per_instance + cpus_per_instance, cpus_per_instance)
        else:
            core.thread_bind(socket_id, cpus_per_socket, i * cpus_per_instance, cpus_per_instance)
        ...
        if args.accuracy:
            tcount = (self.total_samples.value + args.max_batchsize - 1 ) // args.max_batchsize
        else:
            tcount = (self.min_query_count + args.max_batchsize - 1) // args.max_batchsize
        ...
        while True:
            ...
            if (tcount - self.finished_samples.value > total_instances):
                self.barrier.wait()
                if self.socket_id == 0 and i == 0:
                    self.finished_samples.value += total_instances
    ...
    def run(self):
        ...
        model = self.backend().load(self.args.model_path, self.args.inputs, self.args.outputs)

        if self.socket_id == 0 and cpus_per_instance >= (cpus_per_socket // 2):
            num_ins = self.threads // cpus_per_instance + 1
        else:
            num_ins = self.threads // cpus_per_instance

        for i, _ in enumerate(range(num_ins)):
            worker = mp.Process(target=self.handle_tasks, args=(model, self.task_queue, self.result_queue, i, self.socket_id, self.args))
...
class QueueRunner:
    ...
    def enqueue(self, query_id, idx):
        global qcount
        self.issues += 1
        self.ids.append(query_id[0])
        self.idxes.append(idx[0])

        if self.issues == qcount:
            self.inQueue.put(Item(self.ids, self.idxes, self.issues))
        elif self.issues % self.max_batchsize == 0:
            self.inQueue.put(Item(self.ids, self.idxes, self.issues))
            self.ids = []
            self.idxes = []
...
def main():
    ...
    if cpus_per_instance >= (cpus_per_socket // 2):
        total_instances = cpus_per_socket * num_sockets // cpus_per_instance
    else:
        total_instances = cpus_per_socket * num_sockets // cpus_per_instance - 1
    ...
    consumers = [Consumer(inQueue, outQueue, dsQueue, lock, init_counter, total_samples, finished_samples, barrier, i, args, settings.min_query_count)
    ...
    def load_query_samples(sample_list):
        ...
        while init_counter.value < num_sockets + total_instances: time.sleep(2)
