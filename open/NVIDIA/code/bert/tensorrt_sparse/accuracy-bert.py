...
from code.bert.tensorrt_sparse.evaluate import f1_score
from code.bert.tensorrt_sparse.helpers.data_processing import get_predictions, read_squad_json, convert_example_to_features
            ...
            for q_idx, qas in enumerate(paragraph["qas"]):
                if sample_idx < len(predictions):
                    ...
                    for answer in answers:
                        score = max(score, f1_score(predictions[sample_idx], answer["text"]))
                    score_total += score
                sample_idx += 1

    return score_total / len(predictions) * 100
