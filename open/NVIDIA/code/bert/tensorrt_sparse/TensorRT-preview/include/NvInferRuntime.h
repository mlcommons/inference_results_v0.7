...
public:
    ...
    virtual bool setWeights(const char* layerName, WeightsRole role, Weights weights) noexcept = 0;
    ...
    //!
    //! \brief Get description of all weights that could be refitted.
    //!
    //! \param size The number of items that can be safely written to a non-null layerNames or roles.
    //! \param layerNames Where to write the layer names.
    //! \param roles Where to write the weights roles.
    //!
    //! \return The number of Weights that could be refitted.
    //!
    //! If layerNames!=nullptr, each written pointer points to a string owned by
    //! the engine being refitted, and becomes invalid when the engine is destroyed.
    //!
...
public:
    ...
    //!
    //! \param names The name of the weights to be refitted.
    ...
    //!
    //! \brief Get names of all weights that could be refitted.
    //!
    //! \param size The number of weights names that can be safely written to.
    //! \param weightsNames The names of the weights to be updated, or nullptr for unnamed weights.
    //!
...
public:
    ...
    //!
    //! \brief Return true if engine can be refitted.
    //!
